#!/usr/bin/env python3
'''
* Standalone kasan-monitor-script distribution + launch tool.

* Kills any already-running kasan-monitor-script process on the AP (pkill -f),
then SCPs the local kasan-monitor-script.sh (in this same folder) to
/root/kasan-monitor-script.sh on each AP in AP_LIST below (or the --ap
...
log = logging.getLogger('kasan-monitor-deploy')
...
# ---------------------------------------------------------------------------
# Local kasan-monitor script + remote destination
# ---------------------------------------------------------------------------
LOCAL_SCRIPT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'kasan-monitor-script.sh')
REMOTE_SCRIPT_PATH = '/root/kasan-monitor-script.sh'
...
def startScriptOnAp(host, username, password, remotePath, timeout=30):
    '''Open an interactive SSH session, launch the kasan-monitor script in
    the background with nohup (output redirected to /dev/null, so it keeps
    ...
    try:
        ...
        log.info(f'[{host}] kasan-monitor-script started with pid={pid}')
        return pid
    finally:
        try:
            child.sendline('exit')
...
def killExistingScriptOnAp(host, username, password, remotePath, timeout=30):
    '''Kill any already-running kasan-monitor-script process on the AP before
    ...
    try:
        _authenticate(child, password, host, promptPattern=ROOT_PROMPT, timeout=timeout)
        log.info(f'[{host}] [ROOT] Checking for existing kasan-monitor-script process')
        ...
        if not pids:
            log.info(f'[{host}] No existing kasan-monitor-script process found')
            return

        log.info(f'[{host}] Found existing kasan-monitor-script process(es) pid={",".join(pids)} — killing')
        killCmd = f'kill -9 {" ".join(pids)}'
        log.info(f'[{host}] [ROOT] {killCmd}')
        child.sendline(killCmd)
        child.expect(ROOT_PROMPT, timeout=timeout)
        log.debug(f'[{host}] kill output: {child.before!r}')
        log.info(f'[{host}] Killed old kasan-monitor-script pid={",".join(pids)}')
    finally:
        try:
            child.sendline('exit')
        except Exception:
...
_EXAMPLES = '''\
examples:
  python3 kasan-monitor-deploy.py
  python3 kasan-monitor-deploy.py --ap 10.86.205.157
  python3 kasan-monitor-deploy.py --ap 10.86.205.157,10.86.205.158
  python3 kasan-monitor-deploy.py --debug
'''
...
def main():
    ...
    if not os.path.isfile(LOCAL_SCRIPT_PATH):
        log.error(f'kasan-monitor-script.sh not found at {LOCAL_SCRIPT_PATH}')
        sys.exit(1)
    ...
    print('\n' + '-' * 40)
    print('Deploy + start kasan-monitor-script on AP(s)')
    print('-' * 40)
    ...
    if errors:
        sys.exit(1)
    log.info(f'kasan-monitor-script deployed and started successfully on all {len(apList)} AP(s)')
